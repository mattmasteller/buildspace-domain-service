...
contract Domains is ERC721URIStorage {
    ...
    // Mappings to store their names and values
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        require(len > 0);
        if (len == 3) {
            return 5 * 10**17; // 5 MATIC = 5_000_000_000_000_000_000 (18 decimals). We're going with 0.5 Matic cause the faucets don't give a lot
        } else if (len == 4) {
    ...
    function register(string calldata name) public payable {
        ...
        // Check if enough Matic was paid in the txn
        uint256 _price = price(name);
        require(msg.value >= _price, "Not enough Matic paid");

        // Combine the name passed into the function with the TLD
        string memory _name = string(abi.encodePacked(name, ".", tld));
        console.log("Domain name: %s", _name);
